...
df = df %>%
  group_by(Date) %>%
  summarise(load = mean(load)) ## 49337 records  ## 5 duplicate hours
...
######## 3.2 create lag for two days before, three days before, one week before, etc. since lag 48, 72 is significant, and weekly patern is obvious
n = nrow(df)
l0 = c(df$load)
...
## use the past 3 years' data to train the model
...
for( i in seq(24*0,24*364,24)){
  ...
  ## Use 40 hours data as the validation set
...
importance = xgb.importance(colnames(train_m), model = micro_model)
